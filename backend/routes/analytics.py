...
async def get_user_data_schema(
    ...
    try:
        cur = conn.execute(
            "SELECT DISTINCT field FROM user_data ORDER BY field"
        )
            ...
            if f == "tg_user_id":
                ...
                if first is not None:
                    try:
                        ...
                        sample[f] = list(dict.fromkeys(str(v) for v in vals if v is not None))
                        field_types[f] = "строка"
                else:
                    sample[f] = []
                    field_types[f] = "строка"
        ...
        conn.close()
    return {"fields": fields, "sample": sample, "fieldTypes": field_types}
